"""
FastVLA Production Benchmark — Modal L4 & T4 — Real model weights
==================================================================
Runs real OpenVLA-7B, SmolVLA, OlmoVLA from the registry on both GPUs.
  - Loads true HF weights (Llama-2-7B requires HF_TOKEN with access)
  - Measures: inference latency, training step time, peak VRAM, control Hz
  - Logs everything to W&B
  - Per-model exception isolation: failure on one model does not stop others

Usage:
    WANDB_API_KEY=... HF_API_KEY=... \\
    MODAL_TOKEN_ID=... MODAL_TOKEN_SECRET=... \\
    modal run --detach examples/modal_production_benchmark.py
"""

import os
import json
import time
import modal

_FASTVLA_REPO_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..")
)

_image = (
    modal.Image.from_registry("nvcr.io/nvidia/pytorch:24.01-py3", add_python="3.11")
    .apt_install("git")
    .pip_install(
        "transformers>=4.43.0",
        "accelerate>=0.34.0",
        "bitsandbytes>=0.43.0",
        "peft>=0.12.0",
        "datasets>=2.20.0",
        "torchvision>=0.17.0",
        "timm>=0.9.12,<1.0.0",  # OpenVLA prismatic code pins timm < 1.0
        "numpy>=1.24.0,<2.0.0",
        "pillow>=10.0.0",
        "triton>=2.3.0",
        "huggingface_hub>=0.26.0",
        "wandb>=0.18.0",
        "psutil",
        "sentencepiece",
    )
    .pip_install("unsloth @ git+https://github.com/unslothai/unsloth.git")
    .add_local_dir(_FASTVLA_REPO_ROOT, "/root/fastvla_src", copy=True)
    .run_commands("pip install /root/fastvla_src")
)

app = modal.App("fastvla-production-benchmark", image=_image)

_secrets = [
    modal.Secret.from_dict({
        "WANDB_API_KEY": os.environ.get("WANDB_API_KEY", ""),
        "HF_TOKEN":      os.environ.get("HF_API_KEY", ""),
        "HF_API_KEY":    os.environ.get("HF_API_KEY", ""),
    })
]

# (preset name, image_size, batch_inf, batch_tr, train_steps, num_inf_iters)
MODELS = [
    ("smolvla",    224, 1, 2, 25, 50),
    ("openvla-7b", 384, 1, 1, 10, 30),
]


def _bench_one(model_name: str, image_size: int, B_inf: int, B_tr: int,
               train_steps: int, num_iters: int, device: str) -> dict:
    import torch
    from fastvla import FastVLAModel

    res = {"model": model_name}
    print(f"\n  ── {model_name} ──────────────────────────────────────")

    t0 = time.time()
    try:
        model = FastVLAModel.from_pretrained(model_name)
    except Exception as exc:
        res["error_load"] = str(exc)
        print(f"    LOAD FAILED: {exc}")
        return res
    res["model_load_s"] = round(time.time() - t0, 2)
    print(f"    Loaded in {res['model_load_s']}s")

    model = model.to(device)
    model.eval()
    if device == "cuda":
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        torch.cuda.reset_peak_memory_stats()

    T = 32
    batch_inf = {
        "pixel_values":   torch.randn(B_inf, 1, 3, image_size, image_size, device=device),
        "input_ids":      torch.randint(0, 1000, (B_inf, T), device=device),
        "attention_mask": torch.ones(B_inf, T, device=device),
    }

    try:
        with torch.no_grad():
            for _ in range(3):
                model(**batch_inf)
        if device == "cuda":
            torch.cuda.synchronize()
        latencies = []
        with torch.no_grad():
            for _ in range(num_iters):
                t = time.perf_counter()
                model(**batch_inf)
                if device == "cuda":
                    torch.cuda.synchronize()
                latencies.append((time.perf_counter() - t) * 1000)
        avg = sum(latencies) / len(latencies)
        res.update({
            "inference_avg_ms": round(avg, 2),
            "inference_min_ms": round(min(latencies), 2),
            "inference_p95_ms": round(sorted(latencies)[int(0.95 * num_iters)], 2),
            "control_hz":       round(1000.0 / avg, 2),
            "vram_after_inf_gb": round(
                torch.cuda.memory_allocated() / 1024**3 if device == "cuda" else 0, 3
            ),
        })
        print(f"    INF avg {avg:.1f} ms  |  {res['control_hz']} Hz  |  "
              f"VRAM {res['vram_after_inf_gb']} GB")
    except Exception as exc:
        res["error_inference"] = str(exc)
        print(f"    INFERENCE FAILED: {exc}")

    try:
        batch_tr = {
            "pixel_values":   torch.randn(B_tr, 1, 3, image_size, image_size, device=device),
            "input_ids":      torch.randint(0, 1000, (B_tr, T), device=device),
            "attention_mask": torch.ones(B_tr, T, device=device),
            "labels":         torch.randn(B_tr, 7, device=device),
        }
        optimizer = torch.optim.AdamW(
            [p for p in model.parameters() if p.requires_grad], lr=1e-4
        )
        model.train()
        step_times = []
        for step in range(train_steps):
            t = time.perf_counter()
            optimizer.zero_grad()
            _, loss = model(**batch_tr)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            if device == "cuda":
                torch.cuda.synchronize()
            elapsed = time.perf_counter() - t
            if step >= 2:
                step_times.append(elapsed)
            if step % 5 == 0:
                print(f"      step {step:3d}  loss={loss.item():.4f}  "
                      f"{elapsed*1000:.0f} ms")
        avg = sum(step_times) / len(step_times) * 1000
        peak = torch.cuda.max_memory_allocated() / 1024**3 if device == "cuda" else 0
        reserved = torch.cuda.max_memory_reserved() / 1024**3 if device == "cuda" else 0
        res.update({
            "train_step_avg_ms":  round(avg, 2),
            "train_its":          round(1000.0 / avg, 2),
            "peak_vram_alloc_gb": round(peak, 3),
            "peak_vram_res_gb":   round(reserved, 3),
        })
        print(f"    TRAIN {avg:.0f} ms/step  |  {res['train_its']} it/s  |  "
              f"peak VRAM {peak:.2f} GB")
    except Exception as exc:
        res["error_train"] = str(exc)
        print(f"    TRAIN FAILED: {exc}")

    del model
    if device == "cuda":
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
    return res


def _run_benchmark(gpu_label: str) -> dict:
    import torch
    import wandb

    device_name = torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu"
    vram_total  = (
        torch.cuda.get_device_properties(0).total_memory / 1024**3
        if torch.cuda.is_available() else 0
    )
    device = "cuda" if torch.cuda.is_available() else "cpu"

    print(f"\n{'='*60}")
    print(f"  FastVLA Production Benchmark — {gpu_label}")
    print(f"  Device  : {device_name}")
    print(f"  VRAM    : {vram_total:.1f} GB")
    print(f"{'='*60}")

    wandb.init(
        project="fastvla-production-benchmark",
        name=f"prod-{gpu_label}-{int(time.time())}",
        config={"gpu": gpu_label, "device": device_name, "vram_gb": vram_total},
        reinit=True,
    )

    all_results = {
        "gpu_label": gpu_label,
        "device_name": device_name,
        "vram_total_gb": vram_total,
        "models": {},
    }

    for name, img_sz, b_inf, b_tr, train_steps, num_iters in MODELS:
        per = _bench_one(name, img_sz, b_inf, b_tr, train_steps, num_iters, device)
        all_results["models"][name] = per
        wandb.log({f"{name}/{k}": v for k, v in per.items()
                   if isinstance(v, (int, float, bool))})

    wandb.log({"summary": json.dumps(all_results["models"])})
    wandb.finish()

    print(f"\n  DONE {gpu_label}")
    for name, per in all_results["models"].items():
        print(f"\n    [{name}]")
        for k, v in per.items():
            print(f"      {k:<28} {v}")
    return all_results


@app.function(gpu="L4", timeout=3600, secrets=_secrets)
def benchmark_l4() -> dict:
    return _run_benchmark("L4")


@app.function(gpu="T4", timeout=3600, secrets=_secrets)
def benchmark_t4() -> dict:
    return _run_benchmark("T4")


@app.local_entrypoint()
def main():
    modal.enable_output()
    print("\n🚀  FastVLA production benchmark — Modal L4 + T4 (real weights)\n")

    futures = {
        "L4": benchmark_l4.spawn(),
        "T4": benchmark_t4.spawn(),
    }

    results = {}
    for label, fut in futures.items():
        print(f"⏳  Waiting for {label} ...")
        try:
            results[label] = fut.get()
        except Exception as exc:
            print(f"❌  {label} failed: {exc}")
            results[label] = {"error": str(exc)}

    print("\n" + "=" * 60)
    print(json.dumps(results, indent=2))

    with open("production_benchmark_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n📄  Results → production_benchmark_results.json")

    # Comparison table per model
    for model_name in [m[0] for m in MODELS]:
        print(f"\n┌────────────── {model_name} ──────────────┐")
        print(f"│  Metric                  │  L4          │  T4          │")
        print(f"├──────────────────────────┼──────────────┼──────────────┤")
        for m in ["inference_avg_ms", "control_hz", "train_step_avg_ms",
                  "train_its", "peak_vram_alloc_gb"]:
            l4v = results.get("L4", {}).get("models", {}).get(model_name, {}).get(m, "n/a")
            t4v = results.get("T4", {}).get("models", {}).get(model_name, {}).get(m, "n/a")
            fmt = lambda v: f"{v:.2f}" if isinstance(v, float) else str(v)
            print(f"│  {m:<24}  │  {fmt(l4v):<12} │  {fmt(t4v):<12} │")
        print(f"└──────────────────────────┴──────────────┴──────────────┘")
