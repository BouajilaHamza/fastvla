...
print("=" * 80)
print("FASTVLA DTYPE COMPATIBILITY TEST SUITE")
print("=" * 80)
...
test("Import action head adapters", test_import_adapters)
test("Import fusion kernel", test_import_fusion)
test("Import data collator", test_import_collator)


# ── TritonActionHead Tests ─────────────────────────────────────────────

print("\n" + "=" * 80)
print("2. TRITON ACTION HEAD TESTS")
print("=" * 80)
...
def test_triton_cpu_fp32_forward():
    head = TritonActionHead(input_dim=768, hidden_dim=256, output_dim=7)
...
print("\n" + "=" * 80)
print("3. DISCRETE ACTION HEAD TESTS")
print("=" * 80)

from fastvla.adapters.action_head import DiscreteActionHead


def test_discrete_cpu_fp32_forward():
    head = DiscreteActionHead(input_dim=768, action_dim=7, hidden_dim=256, num_bins=256)
    x = torch.randn(2, 768, dtype=torch.float32)
    with torch.no_grad():
        output = head(x)
    assert output.shape == (2, 7), f"Expected shape (2, 7), got {output.shape}"
...
print("\n" + "=" * 80)
print("4. CONTINUOUS ACTION HEAD TESTS")
print("=" * 80)

from fastvla.adapters.action_head import ContinuousActionHead
...
print("\n" + "=" * 80)
print("5. FLOW MATCHING ACTION HEAD TESTS")
print("=" * 80)

from fastvla.adapters.action_head import FlowMatchingActionHead


def test_flow_cpu_fp16_forward():
    """CRITICAL TEST: Mixed precision with FlowMatchingActionHead."""
    head = FlowMatchingActionHead(input_dim=768, action_dim=7, hidden_dim=256)
    x = torch.randn(2, 768, dtype=torch.float16)
    with torch.no_grad():
        output = head(x)  # This would fail without dtype fix
    assert output.shape == (2, 7), f"Expected shape (2, 7), got {output.shape}"
...
print("\n" + "=" * 80)
print("6. VISION-LANGUAGE FUSION TESTS")
print("=" * 80)
...
print("\n" + "=" * 80)
print("8. MIXED PRECISION INTEGRATION TESTS")
print("=" * 80)
