...
def test_triton_action_parity():
    ...
    print(f"Forward Max Diff: {torch.max(torch.abs(out_torch - out_triton)).item():.2e}")
    ...
    print(f"Backward Grad_X Max Diff: {torch.max(torch.abs(grad_x_torch - grad_x_triton)).item():.2e}")
    assert torch.allclose(grad_x_torch, grad_x_triton, atol=1e-5), "Backward parity failed!"
