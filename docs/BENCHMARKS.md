...
> **Caveat — OpenVLA-7B vision tower:** the loader at `fastvla/adapters/vision.py::OpenVLAFusedVisionAdapter.from_pretrained` now cascades four strategies (`AutoModelForImageTextToText` → `AutoModelForVision2Seq` → dynamic class load via `auto_map` → plain `AutoModel`) before falling back to SigLIP-so400m-384. In the production-benchmark image, **all four still fail**: the upstream OpenVLA prismatic code is pinned to `transformers==4.40.1` / `timm<1.0.0`, while FastVLA's runtime needs newer `transformers` (for Unsloth's API) and newer `bitsandbytes` paths. The two pin envelopes do not overlap — `tie_weights()` signature changed in transformers 5.x. So the production-benchmark numbers below characterise a "SigLIP + Llama-2-7B + FastVLA action head" deployment. The standalone baseline image (`examples/modal_baseline_benchmark.py`) does pin to 4.40.1 and successfully loads the full prismatic backbone for inference (138 ms / 7.25 Hz / 14.1 GB on L4 bf16). Closing the version gap is tracked alongside [issue #2](https://github.com/BouajilaHamza/fastvla/issues/2).
