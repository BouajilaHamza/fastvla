...
def main():
    ...
    if args.vision_dtype == "float16":
        ...
        model.base_model.model.vision_backbone = (
            model.base_model.model.vision_backbone.half()
        )
    ...
    for step in range(args.steps):
        ...
        px, iid, am, labels = generate_robotics_batch(
            args.batch, args.seq_len, args.image_size, dtype=vision_dtype, device=device
        )
    ...
    print(
        f"  Loss:              {losses[0]:.4f} → {losses[-1]:.4f} (Δ={losses[0] - losses[-1]:.4f})"
    )
