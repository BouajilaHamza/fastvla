...
def main():
    ...
    with torch.no_grad():
        for i in range(5):
            ...
            px = IMAGE_TRANSFORM(img).unsqueeze(0).to(device, dtype=torch.float32)  # [1,3,224,224]
            ...
            tokenized = tokenizer(
                [ds_info['instruction']], return_tensors="pt", padding=True,
            ...
            out = model(pixel_values=px, input_ids=input_ids, attention_mask=attention_mask, labels=labels)
    ...
    for step in range(args.steps):
        ...
        # Forward
        outputs = model(pixel_values=px, input_ids=input_ids, attention_mask=attention_mask, labels=labels)
        loss = outputs.loss
        loss.backward()
