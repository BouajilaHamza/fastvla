...
def prepare_batch(batch, device="cuda"):
    ...
    images = []
    for img1, img2 in zip(batch["observation.images.image"], batch["observation.images.image2"]):
    ...
    pixel_values = torch.stack(images).to(device, dtype=torch.float32)  # [B, 6, 224, 224]
    
    # Convert actions to tensor
    actions = torch.tensor(batch["action"], dtype=torch.float32, device=device)  # [B, 7]
...
def main():
    ...
    print(f"  Sample: image={len(ds[0]['observation.images.image'].size)}, "
    ...
    with torch.no_grad():
        for i in range(0, min(20, val_size)):
            ...
            out = model(pixel_values=px, input_ids=input_ids, attention_mask=attention_mask)
            val_losses.append(out.loss.item())
    
    pre_val_loss = sum(val_losses) / len(val_losses)
    print(f"  ✅ Pre-training val loss: {pre_val_loss:.4f} ({len(val_losses)} samples)\n")
    model.train()
    ...
    for step in range(args.steps):
        ...
        # Forward
        outputs = model(pixel_values=px, input_ids=input_ids, attention_mask=attention_mask)
        loss = outputs.loss
        loss.backward()
    ...
    print(f"RESULTS")
    print(f"{'='*70}")
    print(f"  Dataset:           LIBERO (HuggingFaceVLA)")
