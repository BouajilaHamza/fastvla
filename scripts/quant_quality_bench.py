...
def generate_batch(batch_size, seq_len=64, image_size=224, device="cuda"):
    ...
    pixel_values = torch.randn(
        batch_size, 6, image_size, image_size, device=device, dtype=torch.float32
    )
...
def generate_fixed_val_set(num_samples=20, seq_len=64, image_size=224, device="cuda"):
    ...
    for _ in range(num_samples):
        ...
        iid = torch.randint(
            100, 32000, (1, seq_len), device=device, generator=generator
        )
...
def evaluate(model, val_batches):
    ...
    with torch.no_grad():
        for px, iid, am, labels in val_batches:
            out = model(
                pixel_values=px, input_ids=iid, attention_mask=am, labels=labels
            )
...
def load_model(quant_mode="q4", image_size=224):
    ...
    print(
        f"  ✅ Loaded in {load_time:.1f}s | VRAM: {vram_load:.2f} GB | Params: {total_params:,}"
    )
...
def run_experiment(
    ...
    print(f"\n{'=' * 70}")
    print(
        f"EXPERIMENT: {quant_mode.upper()}-LoRA | Steps={steps} | LR={lr} | BS={batch_size}"
    )
    print(f"{'=' * 70}\n")

    model, vram_load, load_time, trainable, total_params = load_model(
        quant_mode, image_size
    )
...
def print_comparison(results):
    ...
    print("│ Mode     │ VRAM   │ Step   │ Steps/s │ Pre-val  │ Post-val │ Δ-val    │")
    print("├──────────┼────────┼────────┼─────────┼──────────┼──────────┼──────────┤")

    for r in sorted(results, key=lambda x: {"q4": 0, "q8": 1}.get(x["quant_mode"], 2)):
        q = r["quant_mode"].upper()
        print(
            f"│ {q + '-LoRA':8s} │ {r['load_vram_gb']:5.2f} │ {r['avg_step_ms']:5.0f}ms │ "
    ...
    if len(results) == 2:
        ...
        if q4 and q8:
            speed_ratio = (
                q4["steps_per_sec"] / q8["steps_per_sec"]
                if q8["steps_per_sec"] > 0
                else 0
            )
            ...
            print("\n  Analysis:")
            print(
                f"    Speed:    Q4 is {speed_ratio:.2f}x {'faster' if speed_ratio > 1 else 'slower'} than Q8"
            )
            print(f"    Quality:  Post-val diff = {val_diff:.4f}")

            if val_diff < 0.1:
                print(
                    "    ✅ Q4 and Q8 converge to similar quality → Q4 is sufficient"
                )
            elif val_diff < 0.5:
                print(
                    "    ⚠️  Q4 slightly worse than Q8 → acceptable tradeoff for speed"
                )
            else:
                print(
                    "    ❌ Q4 significantly worse than Q8 → use Q8 for quality-critical tasks"
                )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--quant", type=str, default="q4", choices=["q4", "q8", "all"])
