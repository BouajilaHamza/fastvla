...
def _action_fwd_kernel(
    ...
    for h_start in range(0, H, BLOCK_H):
        ...
        for d_start in range(0, D, 32):
            ...
            mask_d = off_d < D
            x = tl.load(
                x_ptr + off_b[:, None] * stride_xb + off_d[None, :] * stride_xd,
                mask=mask_b[:, None] & mask_d[None, :],
                other=0.0,
            )
            w1 = tl.load(
                w1_ptr + off_d[:, None] * stride_w1d + off_h[None, :] * stride_w1h,
        ...
        w2 = tl.load(
            w2_ptr + off_h[:, None] * stride_w2h + off_a[None, :] * stride_w2a,
    ...
    tl.store(
        out_ptr + off_b[:, None] * stride_ob + off_a[None, :] * stride_oa,
        out,
        mask=mask_b[:, None] & mask_a[None, :],
    )
