            ...
            for i in range(self.chunk_size):
                ...
                if look_ahead_item.get("episode_id", curr_ep) != curr_ep:
                    actions.append(
                        actions[-1]
                        if actions
                        else look_ahead_item.get(self.action_key, [])
                    )
                else:
                ...
                data.append(
                    {
                        ...
                        "state": np.concatenate(
                            [f["observations/qpos"][t], f["observations/qvel"][t]]
                        ),
                        "action": f["actions"][t],
    ...
    def _load_data(self):
        ...
        for item in hf_ds:
            ...
            instruction = (
                ...
                or item.get("language_instruction")
                or "push the block to the goal"
            )
        ...
        if not data:
            logger.warning(
                f"⚠️ No valid data found in {self.data_path}. Fallback to raw keys."
            )
            sample = hf_ds[0]
            print(f"DEBUG: Dataset Sample Keys: {list(sample.keys())}")
            # Just take the first image and state we find
            img_key = next(
                (k for k in sample.keys() if "image" in k or "rgb" in k), None
            )
            if img_key:
                print(f"DEBUG: Selected fallback img_key: {img_key}")
            for item in hf_ds:
                data.append(
                    {
                        ...
                        "instruction": item.get(
                            "instruction", "push the block to the goal"
                        ),
                    }
                )
...
def get_dataset(
    dataset_name: str, data_path: Optional[str] = None, **kwargs
) -> RoboticsDataset:
    ...
    if name_lower not in dataset_map:
        ...
        return LeRobotDataset(
            dataset_name if data_path is None else data_path, **kwargs
        )
